...
//	err := license.SetMeteredKey("b8691e842359b46392642f94402109ddab1bc37da7411ba9e30324b9a9a35ced")
//	if err != nil {
//		panic(err)
//	}
//}
...
func handleCodeReview(w http.ResponseWriter, r *http.Request) {
	...
	for _, fileHeader := range files {
		...
		resp, err := chat.Do(
			...
			&qianfan.ChatCompletionRequest{
				Messages: []qianfan.ChatCompletionMessage{
					qianfan.ChatCompletionUserMessage("请帮我审查这段代" + string(content) + "给出这段代" +
...
func handleDocQuery(w http.ResponseWriter, r *http.Request) {
	//fmt.Println("收到文档查询请求")
	if r.Method != http.MethodPost {
		fmt.Println("只支持POST请求")
		http.Error(w, "只支持POST请求", http.StatusMethodNotAllowed)
		return
	}
	...
	//fmt.Printf("文件保存的绝对路径是: %s\n", absolutePath)

	//===============================================================================================
	if err != nil {
		//fmt.Println("1.获取上传的文件失败")
	...
	content, err := readPdf(absolutePath) // Read local pdf file
	if err != nil {
		fmt.Println("4.读取文件内容失败")
		http.Error(w, "文件读取失败: "+err.Error(), http.StatusInternalServerError)
	}
	// 构建提示词
	//fmt.Println(string(content))
	prompt := fmt.Sprintf("请基于以下文档内容回答问题：\n\n文档内容：\n" + content + "\n问题：\n" + input)
...
func main() {
	...
	//注册新的文档查询接口
	http.HandleFunc("/api/doc_query", enableCORS(handleDocQuery))
	err := initDB()
	if err != nil {
